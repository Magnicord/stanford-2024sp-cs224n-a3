...
def compute_corpus_level_bleu_score(
    ...
    detokened_refs = [
        "".join(pieces).replace("▁", " ") for pieces in references
    ]
    detokened_hyps = [
        "".join(hyp.value).replace("▁", " ") for hyp in hypotheses
    ]
    ...
    while True:
        ...
        for src_sents, tgt_sents in batch_iter(
            ...
            if train_iter % log_every == 0:
                writer.add_scalar(
                    "loss/train", report_loss / report_tgt_words, train_iter
                )
                writer.add_scalar(
                    "perplexity/train",
            ...
            if train_iter % valid_niter == 0:
                writer.add_scalar(
                    "loss/val", cum_loss / cum_tgt_words, train_iter
                )
                print(
                ...
                writer.add_text(
                    ...
                    format_example_sentence(
                        example_sentence_src,
                        example_sentence_tgt,
                        example_hypothesis_beam,
                        train_iter,
                    ),
                    train_iter,
                )
                ...
                if is_better:
                    ...
                    torch.save(
                        optimizer.state_dict(), model_save_path + ".optim"
                    )
...
def decode(args: dict[str, str]):
    ...
    if args["TEST_TARGET_FILE"]:
        print(
            "load test target sentences from [{}]".format(
