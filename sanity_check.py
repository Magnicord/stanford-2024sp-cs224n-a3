...
def generate_outputs(model, source, target, vocab):
    ...
    torch.save(
        combined_outputs, "./sanity_check_en_es_data/combined_outputs.pkl"
    )
    ...
    # Run Tests
    with torch.no_grad():
        dec_state_target, o_t_target, e_t_target = model.step(
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    ), "enc_hiddens shape is incorrect: it should be:\n {} but is:\n{}".format(
        enc_hiddens_target.shape, enc_hiddens_pred.shape
    )
    assert np.allclose(
    ...
    ), "dec_init_state[0] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_init_state_target[0].shape, dec_init_state_pred[0].shape
    )
    assert np.allclose(
    ...
    ), "dec_init_state[1] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_init_state_target[1].shape, dec_init_state_pred[1].shape
    )
    assert np.allclose(
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    ), "combined_outputs shape is incorrect: it should be:\n {} but is:\n{}".format(
        combined_outputs_target.shape, combined_outputs_pred.shape
    )
    assert np.allclose(
...
def question_1f_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    ), "decoder_state[0] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_state_target[0].shape, dec_state_pred[0].shape
    )
    assert np.allclose(
    ...
    ), "decoder_state[1] shape is incorrect: it should be:\n {} but is:\n{}".format(
        dec_state_target[1].shape, dec_state_pred[1].shape
    )
    assert np.allclose(
...
def main():
    ...
    ), "Please update your installation of PyTorch >= 1.6.0. You have version {}.".format(
        torch.__version__
    )
